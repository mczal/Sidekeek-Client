...
function faqController($scope, $http, $sce){
    ...
    $http.get('assets/js/JSON/faq-101.json').then(function(data){
        $scope.dataFAQ = data;
    });
    ...
    $scope.changeView = function(x){
        if (x == "basic") {
            ...
            $scope.sub = "General";
            $http.get('assets/js/JSON/faq-seekr.json').then(function(data){
                $scope.dataFAQ = data;
            });
        } else if (x == "host-start"){
            $scope.head = "Host Question";
            $scope.sub = "Start Hosting";
            $http.get('assets/js/JSON/faq-host-start.json').then(function(data){
                $scope.dataFAQ = data;
            });
        } else{
            $scope.head = "Host Question";
            $scope.sub = "profile settings";
            $http.get('assets/js/JSON/faq-host-profile.json').then(function(data){
                $scope.dataFAQ = data;
            });
        }
    }
